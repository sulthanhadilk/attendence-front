...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!storedToken || !user) {
      ...
      setMsg('ΓÜá∩╕Å Please login as admin to access this dashboard');
      return;
    }
    ...
    try {
      ...
      if (userData.role !== 'admin') {
        setMsg('ΓÜá∩╕Å Admin access required');
        return;
      }
      setToken(storedToken);
    } catch (error) {
      setMsg('ΓÜá∩╕Å Invalid session. Please login again.');
      return;
    }
  }, []);
  ...
  const loadStats = async () => {
    try {
      ...
      setMsg('Γ¥î Failed to load dashboard statistics');
    } finally {
      setLoading(false);
    }
  }
  ...
  const createUser = async (e) => {
    ...
    try {
      ...
      setMsg(`Γ£à User created successfully! ID: ${res.data.user._id}`)
      ...
      setMsg(`Γ¥î ${err.response?.data?.msg || err.message}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const exportAttendance = async () => {
    try {
      ...
      setMsg(`Γ£à ${res.data.message}. Records exported: ${res.data.recordCount}`);
      ...
      setMsg(`Γ¥î ${err.response?.data?.msg || 'Failed to export attendance data'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const markSampleAttendance = async () => {
    try {
      ...
      if (studentsRes.data.length === 0) {
        setMsg('Γ¥î No students found. Please create some students first.');
        return;
      }
      ...
      setMsg(`Γ£à Sample attendance marked! ${attendanceData.filter(a => a.status === 'present').length} present, ${attendanceData.filter(a => a.status === 'absent').length} absent`);
      ...
      setMsg(`Γ¥î ${err.response?.data?.msg || 'Failed to mark attendance'}`);
    } finally {
      setLoading(false);
    }
  };
            ...
            <form onSubmit={createUser}>
              ...
              <div className="form-group">
                ...
                >
                  ...
                  <option value="admin">≡ƒæ¿ΓÇì≡ƒÆ╝ Administrator</option>
                </select>
              </div>

              {msg && (
                <div className={`alert ${msg.includes('Γ£à') ? 'alert-success' : 'alert-error'}`}>
                  {msg}
                  <button 
                    type="button" 
